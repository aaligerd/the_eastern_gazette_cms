...
const createStoryAsDraft=async(req,res)=>{
    ...
    try {
        ...
        return res.status(201).json({msg:'Story created as draft successfully'});
    } catch (error) {
        console.log(error);
...
const updateStoryAndSaveById=async(req,res)=>{
    const { blog_id, title, content, category, subcategory, lables, seo_title, seo_headline, seo_desc, seo_keywords, seo_url_slug, thumbnail,updated_by,updated_at } = req.body;
    console.log(req.body);
    ...
    try {
        await db.promise().query(updateStoryQ, [title,JSON.stringify(content), category, subcategory, lables.join(','), seo_title, seo_headline, seo_desc, seo_keywords, seo_url_slug, thumbnail,updated_at,updated_by, blog_id]);
...
module.exports={createStoryAsDraft,getStoryForCMSDashboard,getStoryForCmsById,makeBlogUnpublishedById,updateStoryAndSaveById};